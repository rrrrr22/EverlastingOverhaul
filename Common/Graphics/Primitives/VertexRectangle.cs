...
namespace EverlastingOverhaul.Common.Graphics.Primitives {
	public class VertexRectangle {
		...
		public void Draw(Vector2 position, Color color = default, Vector2 size = default, float rotation = 0, Vector2 rotationCenter = default, float? width = null) {
		...
		public void DrawLine(Vector2 position, Color color = default, float rotation = 0, Vector2 rotationCenter = default, float width1 = 0, float width2 = 0, float length = 32)
		{
			vertices[2].Position = new Vector3((position + new Vector2(0,-(width1 / 2))).RotatedBy(rotation, rotationCenter), 0);
			vertices[1].Position = new Vector3((position + new Vector2(length,width2 / 2)).RotatedBy(rotation, rotationCenter), 0);
			vertices[0].Position = new Vector3((position + new Vector2(0,width1 / 2)).RotatedBy(rotation, rotationCenter), 0);
			vertices[3].Position = new Vector3((position + new Vector2(length,-(width2/ 2))).RotatedBy(rotation, rotationCenter), 0);
